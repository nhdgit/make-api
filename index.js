...
// Ajouter la route pour vérifier si un client a cliqué sur le lien
app.post('/verify-click', (req, res) => {
  const { key } = req.body;

  if (!key) {
    return res.status(400).send({ message: "L'ID client (key) est manquant." });
  }

  // Pour l'instant, on suppose que si cette route est appelée, le lien a été cliqué.
  // On retourne simplement l'ID du client.
  res.status(200).send({ client_id: key });
});

// Route vers capture click
app.get('/track-click', async (req, res) => {
  // Récupération de la clé (key) à partir de l'URL de la requête
  const key = req.query.key;

  if (key) {
    console.log(`Clic enregistré pour le client : ${key}`);
    
    // Appel au webhook Make.com avec la clé
    const webhookUrl = 'https://hook.eu2.make.com/u64ngani62prgtf88unznvv3kmj9hhsu';
    try {
      await axios.post(webhookUrl, { key }); // Envoi de la clé au webhook Make.com
      console.log('Webhook Make.com déclenché avec succès.');
    } catch (error) {
      console.error('Erreur lors de l\'appel au webhook :', error);
    }
  } else {
    console.log("Aucune clé (key) n'a été fournie.");
  }
